...
import 'package:studymat_app/components/AddMatScreen/MatListing.dart';
    ...
    child: Column(
      children: institutionData.keys.map((i) {
        return ExpansionTile(
          title: Text(i),
          children:
              (institutionData[i]! as Map<dynamic, dynamic>).keys.map((j) {
            return ExpansionTile(
              title: Text("  $j"),
              children: (institutionData[i][j]! as Map<dynamic, dynamic>)
                  .keys
                  .map((k) {
                return ExpansionTile(
                  title: Text("    $k"),
                  children: (institutionData[i][j][k] as Map<dynamic, dynamic>)
                      .keys
                      .map((l) {
                    return ExpansionTile(
                      title: Text("      $l"),
                      children:
                          (institutionData[i][j][k][l] as Map<dynamic, dynamic>)
                              .keys
                              .map((m) {
                        return ExpansionTile(
                          title: Text("        $m"),
                          children: (institutionData[i][j][k][l][m]
                            ...
                            return ExpansionTile(
                              title: Text("          ${type}s"),
                              children: (institutionData[i][j][k][l][m][type]
                                      as List<Mat>)
                                  .map((mat) {
                                return ListTile(
                                  onTap: () async {
                                    if (type == "pdf") {
                                      ...
                                      final file =
                                          await PDFApi.loadNetwork(mat.link);
                                      ...
                                      if (await canLaunchUrl(
                                          Uri.parse(mat.link))) {
                                        await launchUrl(Uri.parse(mat.link));
                                      }
                                    }
                                  },
                                  title: Text("          ${mat.name}"),
